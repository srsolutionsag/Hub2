...
class CourseOriginConfigFormGUI extends OriginConfigFormGUI {
	...
	protected function addPropertiesNew() {
		...
		$cb = new ilCheckboxInputGUI(self::plugin()->translate('crs_prop_activate'), $this->prop(CourseProperties::SET_ONLINE));
		$cb = new ilCheckboxInputGUI(self::plugin()->translate('crs_prop_activate'), $this->prop(CourseProperties::SET_ONLINE));
		...
		$send_mail = new ilCheckboxInputGUI(self::plugin()
			->translate('crs_prop_send_notification'), $this->prop(CourseProperties::SEND_CREATE_NOTIFICATION));
		...
		$notification_subject = new ilTextInputGUI(self::plugin()
			->translate('crs_prop_notification_subject'), $this->prop(CourseProperties::CREATE_NOTIFICATION_SUBJECT));
		$notification_subject->setValue($this->origin->properties()->get(CourseProperties::CREATE_NOTIFICATION_SUBJECT));

		$send_mail->addSubItem($notification_subject);
		$notification_body = new ilTextAreaInputGUI(self::plugin()
			->translate('crs_prop_notification_body'), $this->prop(CourseProperties::CREATE_NOTIFICATION_BODY));
		...
		$notification_from = new ilEMailInputGUI(self::plugin()
			->translate('crs_prop_notification_from'), $this->prop(CourseProperties::CREATE_NOTIFICATION_FROM));
	...
	protected function addPropertiesUpdate() {
		...
		$cb = new ilCheckboxInputGUI(self::plugin()->translate('crs_prop_move'), $this->prop(CourseProperties::MOVE_COURSE));
		$cb->setChecked($this->origin->properties()->get(CourseProperties::MOVE_COURSE));

		$cb->setInfo(self::plugin()->translate('crs_prop_move_info'));
		$this->addItem($cb);

		$cb = new ilCheckboxInputGUI(self::plugin()->translate('crs_prop_reactivate'), $this->prop(CourseProperties::SET_ONLINE_AGAIN));
	...
	protected function addPropertiesDelete() {
		...
		$delete = new ilRadioGroupInputGUI(self::plugin()->translate('crs_prop_delete_mode'), $this->prop(CourseProperties::DELETE_MODE));
		$delete->setValue($this->origin->properties()->get(CourseProperties::DELETE_MODE));

		$opt = new ilRadioOption(self::plugin()->translate('crs_prop_delete_mode_none'), CourseProperties::DELETE_MODE_NONE);
		$delete->addOption($opt);

		$opt = new ilRadioOption(self::plugin()->translate('crs_prop_delete_mode_inactive'), CourseProperties::DELETE_MODE_OFFLINE);
		$delete->addOption($opt);

		$opt = new ilRadioOption(self::plugin()->translate('crs_prop_delete_mode_delete'), CourseProperties::DELETE_MODE_DELETE);
		$delete->addOption($opt);

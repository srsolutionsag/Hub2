...
class OriginConfigFormGUI extends ilPropertyFormGUI {
	...
	public function __construct($parent_gui, IHubConfig $hubConfig, IOriginRepository $originRepository, IOrigin $origin) {
		...
		$this->initForm();
		if (!$origin->getId()) {
			$this->addCommandButton(hub2ConfigOriginsGUI::CMD_CREATE_ORIGIN, self::translate('button_save'));
			$this->setTitle(self::translate('origin_form_title_add'));
		} else {
			$this->addCommandButton(hub2ConfigOriginsGUI::CMD_SAVE_ORIGIN, self::translate('button_save'));
			$this->setTitle(self::translate('origin_form_title_edit'));
		}
		$this->addCommandButton(hub2ConfigOriginsGUI::CMD_CANCEL, self::translate('button_cancel'));
		parent::__construct();
	}
	...
	protected function addNotificationConfig() {
		...
		$h->setTitle(self::translate('origin_form_header_notification'));
		$this->addItem($h);
		$te = new ilTextInputGUI(self::translate('origin_form_field_summary_email'), $this->conf(IOriginConfig::NOTIFICATION_SUMMARY));
		$te->setValue(implode(',', $this->origin->config()->getNotificationsSummary()));
		$te->setInfo(self::translate('origin_form_comma_separated'));
		$this->addItem($te);
		$te = new ilTextInputGUI(self::translate('origin_form_field_notification_email'), $this->conf(IOriginConfig::NOTIFICATION_ERRORS));
	...
	protected function addConnectionConfig() {
		$header = new ilFormSectionHeaderGUI();
		$header->setTitle(self::translate('origin_form_header_connection'));
		$this->addItem($header);
		$ro = new ilRadioGroupInputGUI(self::translate('origin_form_field_conf_type'), $this->conf(IOriginConfig::CONNECTION_TYPE));
		$ro->setValue($this->origin->config()->getConnectionType());
		{
			$db = new ilRadioOption(self::translate('origin_form_field_conf_type_file'), IOriginConfig::CONNECTION_TYPE_FILE, self::translate('origin_form_field_conf_type_file_info'));
			{
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_file_path'), $this->conf(IOriginConfig::FILE_PATH));
				$te->setValue($this->origin->config()->getFilePath());
				$db->addSubItem($te);
			}
			$ro->addOption($db);
			$file = new ilRadioOption(self::translate('origin_form_field_conf_type_db'), IOriginConfig::CONNECTION_TYPE_SERVER, self::translate('origin_form_field_conf_type_db_info'));
			{
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_db_host'), $this->conf(IOriginConfig::SERVER_HOST));
				$te->setValue($this->origin->config()->getServerHost());
				$file->addSubItem($te);
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_db_port'), $this->conf(IOriginConfig::SERVER_PORT));
				$te->setValue($this->origin->config()->getServerPort());
				$file->addSubItem($te);
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_db_username'), $this->conf(IOriginConfig::SERVER_USERNAME));
				$te->setValue($this->origin->config()->getServerUsername());
				$file->addSubItem($te);
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_db_password'), $this->conf(IOriginConfig::SERVER_PASSWORD));
				$te->setValue($this->origin->config()->getServerPassword());
				$file->addSubItem($te);
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_db_database'), $this->conf(IOriginConfig::SERVER_DATABASE));
				$te->setValue($this->origin->config()->getServerDatabase());
				$file->addSubItem($te);
				$te = new ilTextInputGUI(self::translate('origin_form_field_conf_type_db_search_base'), $this->conf(IOriginConfig::SERVER_SEARCH_BASE));
				$te->setValue($this->origin->config()->getServerSearchBase());
				$file->addSubItem($te);
			}
			$ro->addOption($file);
			$external = new ilRadioOption(self::translate('origin_form_field_conf_type_external'), IOriginConfig::CONNECTION_TYPE_EXTERNAL, self::translate('origin_form_field_conf_type_external_info'));
	...
	protected function addSyncConfig() {
		$h = new ilFormSectionHeaderGUI();
		$h->setTitle(self::translate('origin_form_header_sync'));
		$this->addItem($h);
		...
		$te = new ilTextInputGUI(self::translate('origin_form_field_namespace'), 'implementation_namespace');
		$te->setInfo(self::translate('origin_form_field_namespace_info'));
		$te->setValue($this->origin->getImplementationNamespace());
		$te->setRequired(true);
		$this->addItem($te);

		$se = new ilSelectInputGUI(self::translate('com_prop_link_to_origin'), $this->conf(IOriginConfig::LINKED_ORIGIN_ID));
		$options = [ '' => '' ];
		...
		$cb = new ilCheckboxInputGUI(self::translate('com_prop_check_amount'), $this->conf(IOriginConfig::CHECK_AMOUNT));
		$cb->setInfo(self::translate('com_prop_check_amount_info'));
		$cb->setChecked($this->origin->config()->getCheckAmountData());

		$se = new ilSelectInputGUI(self::translate('com_prop_check_amount_percentage'), $this->conf(IOriginConfig::CHECK_AMOUNT_PERCENTAGE));
		$options = [];
		for ($i = 10; $i <= 100; $i += 10) {
			$options[$i] = "$i%";
		}
		...
		$cb = new ilCheckboxInputGUI(self::translate('com_prop_shortlink'), $this->conf(IOriginConfig::SHORT_LINK));
		$cb->setChecked($this->origin->config()->useShortLink());
		$subcb = new ilCheckboxInputGUI(self::translate('com_prop_force_login'), $this->conf(IOriginConfig::SHORT_LINK_FORCE_LOGIN));
		$subcb->setChecked($this->origin->config()->useShortLinkForcedLogin());
		$cb->addSubItem($subcb);
		$this->addItem($cb);

		$te = new ilTextInputGUI(self::translate('origin_from_field_active_period'), $this->conf(IOriginConfig::ACTIVE_PERIOD));
		$te->setInfo(self::translate('origin_from_field_active_period_info'));
	...
	protected function addGeneral() {
		...
		$item = new ilTextInputGUI(self::translate('origin_title'), 'title');
		$item->setValue($this->origin->getTitle());
		$item->setRequired(true);
		$this->addItem($item);
		$item = new ilTextAreaInputGUI(self::translate('origin_description'), 'description');
		$item->setValue($this->origin->getDescription());
		$item->setRequired(true);
		$this->addItem($item);
		if ($this->origin->getId()) {
			$item = new ilNonEditableValueGUI();
			$item->setTitle(self::translate('origin_form_field_usage_type'));
			$item->setValue($this->origin->getObjectType()); // TODO: Translate object type
			$this->addItem($item);
			$item = new ilCheckboxInputGUI(self::translate('origin_form_field_active'), 'active');
			$item->setChecked($this->origin->isActive());
			$this->addItem($item);
		} else {
			$item = new ilSelectInputGUI(self::translate('origin_form_field_usage_type'), 'object_type');
			$item->setRequired(true);
			$options = [];
			foreach (AROrigin::$object_types as $type) {
				$options[$type] = self::translate('origin_object_type_' . $type);
			}
